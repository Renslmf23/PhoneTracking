...
class ImageHandler:
    ...
    def find_phone(self, img):
        ...
        midpoints = markers.find_markers(img)
        if midpoints is None:
            return filtered, resized, None, phone
        for midpoint in midpoints:
            cv2.circle(resized, (midpoint[0], midpoint[1]), 10, (255, 0, 0))
        return filtered, resized, midpoints, phone
    ...
    def calib_hsv(self, image=None, cam_num=0):
        if image is None:
            ...
            cam = device_manager.open_device_by_user_id("0")
        ...
        if self.camSetting is not None:
            ...
            while 1:
                ...
                self.kernel_dilation = np.ones((self.kernel_size, self.kernel_size), np.uint8)
                self.kernel_erosion = np.ones((self.kernel_size_erosion, self.kernel_size_erosion), np.uint8)
                filtered, resized, _, _ = self.find_phone(img_to_check)
